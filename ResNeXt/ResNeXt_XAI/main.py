...
def plot_attributions(attributions, title="Atribuciones"):
    plt.figure(figsize=(10, 5))
    plt.plot(np.arange(attributions.shape[2]), attributions[0, 0, :], label=title, color="blue")
    plt.title(title)
    plt.xlabel('Tiempo (muestras de ECG)')
    plt.ylabel('Atribución')
    plt.legend()
    plt.show()

def interpret_model(model, dataloader, device):
    model.eval()

    def forward_func(x):
        return model(x, raw=True)

    ig = IntegratedGradients(forward_func)
    saliency = Saliency(forward_func)

    sample_input, sample_labels, _ = next(iter(dataloader))
    sample_input = sample_input.transpose(-1, -2)
    sample_input = sample_input.to(device=device, dtype=torch.float32)
    sample_labels = sample_labels.to(device=device)

    input_signal = sample_input[0:1]
    target_label = sample_labels[0].item()

    attributions, delta = ig.attribute(input_signal, target=target_label, return_convergence_delta=True)
    attributions = attributions.cpu().detach().numpy()
    plot_attributions(attributions, title="Atribución por tiempo - Integrated Gradients")

    saliency_map = saliency.attribute(input_signal, target=target_label)
    saliency_map = saliency_map.cpu().detach().numpy()
    plot_attributions(saliency_map, title="Mapa de Saliencia")
...
def fit(
    ...
    *,
    ...
    interpret_model(model, test_dataloader, device)
...
if __name__ == "__main__":
    ...
    ckpt_path = os.path.splitext(model_path)[0] + "__CKPT" + os.path.splitext(model_path)[1]
