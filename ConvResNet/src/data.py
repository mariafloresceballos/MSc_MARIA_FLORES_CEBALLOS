...
class FullDataset(Dataset):
    def __init__(self, files, scaler=None):
        ...
        # Paralelizar lectura de archivos para acelerar carga
        with ThreadPoolExecutor(max_workers=8) as executor:
            data_list = list(tqdm(executor.map(lambda f: Reader.read(f), files), total=len(files), desc="Loading dataset"))

        signals = torch.cat([d[0] for d in data_list], dim=0)
        labels = torch.cat([d[1] for d in data_list], dim=0)
        bpms = torch.cat([d[2] for d in data_list], dim=0)

        self.signals = self.scaler(signals)
        self.labels = labels
        self.bpms = bpms
    ...
    def __getitem__(self, idx):
        return self.signals[idx], self.labels[idx], self.bpms[idx]
    

def fit_scaler_manual(file_list, scaler, batch_size=32, max_workers=8):
    signals_accum = []

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i in tqdm(range(0, len(file_list), batch_size), desc="Fitting scaler manually"):
            batch_files = file_list[i : i + batch_size]
            batch_signals = list(executor.map(lambda f: Reader.read(f)[0], batch_files))
            batch_signals = torch.cat(batch_signals, dim=0)
            signals_accum.append(batch_signals)

    all_signals = torch.cat(signals_accum, dim=0)
...
if __name__ == "__main__":
    ...
    for patient_path in tqdm(patient_dirs[:1000]):
    ...
    for split in ["train", "valid", "test"]:
        ...
        for label in sorted(label_counters[split].keys()):
            ...
            logger.info(f"  Class {label}: {count} segments ({percentage:.2f}%)")
    ...
    for split in ["train", "valid", "test"]:
        ...
        logger.info(f"→ {split.upper()}: {total} segments")
        print(f"→ {split.upper()}: {total} segments")

    # Validación guardado vs etiquetado
    for split in ["train", "valid", "test"]:
        logger.info(f"\n→ {split.upper()}:")
        print(f"\n→ {split.upper()}:")
        ...
        if labeled_segments > saved_segments:
            ...
            logger.warning(f"Warning: {discarded} segments were discarded (not enough to form a full batch of size {batch_size}).")
            ...
            logger.warning("Warning: more segments were saved than labeled. This should not happen.")
            print("Warning: more segments were saved than labeled. This should not happen.")
        else:
            logger.info("All labeled segments were saved successfully.")
