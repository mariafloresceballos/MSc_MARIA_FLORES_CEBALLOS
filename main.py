...
def fit(
    ...
    *,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # for metric_set in [train_metrics_micro, train_metrics_per_class, valid_metrics_micro, valid_metrics_per_class]:
        #     for metric in metric_set.values():
        #         metric.compute()  
        ...
        # # Preparar arrays numpy para sklearn y definir etiquetas únicas para evitar warnings
        # train_all_preds = torch.cat(train_all_preds).numpy()
        # train_all_labels = torch.cat(train_all_labels).numpy()
        # train_labels_union = unique_labels(np.concatenate([train_all_labels, train_all_preds]))
        # train_acc = accuracy_score(train_all_labels, train_all_preds)
        # train_bal_acc = balanced_accuracy_score(train_all_labels, train_all_preds)
        # train_precision, train_recall, train_f1, _ = precision_recall_fscore_support(
        #     train_all_labels, train_all_preds, labels=train_labels_union, average='macro', zero_division=0
        # )

        # print("\nTrain global manual (sklearn) metrics:")
        # print(f"accuracy: {train_acc:.6f}")
        # print(f"balanced accuracy: {train_bal_acc:.6f}")
        # print(f"precision: {train_precision:.6f}")
        # print(f"recall: {train_recall:.6f}")
        # print(f"f1 score: {train_f1:.6f}")

        # valid_all_preds = torch.cat(valid_all_preds).numpy()
        # valid_all_labels = torch.cat(valid_all_labels).numpy()
        # valid_labels_union = unique_labels(np.concatenate([valid_all_labels, valid_all_preds]))
        # valid_acc = accuracy_score(valid_all_labels, valid_all_preds)
        # valid_bal_acc = balanced_accuracy_score(valid_all_labels, valid_all_preds)
        # valid_precision, valid_recall, valid_f1, _ = precision_recall_fscore_support(
        #     valid_all_labels, valid_all_preds, labels=valid_labels_union, average='macro', zero_division=0
        # )

        # print("\nValid global manual (sklearn) metrics:")
        # print(f"accuracy: {valid_acc:.6f}")
        # print(f"balanced accuracy: {valid_bal_acc:.6f}")
        # print(f"precision: {valid_precision:.6f}")
        # print(f"recall: {valid_recall:.6f}")
        # print(f"f1 score: {valid_f1:.6f}")
